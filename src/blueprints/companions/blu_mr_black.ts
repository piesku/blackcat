...
function random_cat_blueprint(owner_is_player: boolean) {
    ...
    return element(cat_blueprints)(owner_is_player);
}

// Mr. Black - Cat Summoner: Spawns random companion cats periodically
export function blueprint_mr_black(owner_is_player: boolean) {
    return [
        ...blueprint_cat_base(
            owner_is_player,
            6, // hp
        ...
        // Spawn random cats every 3 seconds
        spawn_timed(
            () => random_cat_blueprint(owner_is_player),
            3.0, // interval
